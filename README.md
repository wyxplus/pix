...
Existing installations keep pi configuration, models, credentials, packages, extensions, tools and sessions in `~/.pi/agent` / `PI_CODING_AGENT_DIR` until a different storage location is selected. Settings → Memory provides independent personal/project memory switches, data export/import and a data-directory migration option. `PIX_STORAGE_DIR` enables a unified data root; explicit pi directory overrides keep their precedence. See [memory and data portability](MEMORY.md) for supported behavior, validation commands and remaining release gates. Extensions and tools have the same local access as the pi CLI; process isolation is not a sandbox.

Generated answers, progress text and visible thinking output are guided to use the user's conversational language. Explicit language requests and applicable `AGENTS.md` preferences take precedence; quoted documents, code and tool output do not change the conversation language. Standalone summaries and text generation receive bounded language context from the active conversation, while side conversations use their own user messages. This guidance is applied in memory, preserves pi's existing prompts, and does not rewrite project files or previous messages. Model/provider support determines whether thinking output follows the requested language. Interface labels still use the desktop language preference.

The bundled language rules live in [`packages/agent-runtime/resources/AGENTS.md`](packages/agent-runtime/resources/AGENTS.md), which is version-controlled with the application. Both Node builds copy it to `dist/resources/AGENTS.md`; release staging includes it under `sidecar/dist/resources/AGENTS.md` in the macOS (Apple Silicon and Intel), Windows and Linux installers. Pix loads this file relative to its runtime module and appends its contents to pi's prompt, including when a user supplies a custom `SYSTEM.md`. The file is an application resource; it is not copied into the user's workspace or `~/.pi/agent`. Edit it and rebuild to change the bundled defaults. The Sidecar smoke test checks that the shipped file matches the source and reaches the agent's system prompt exactly once.
