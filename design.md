...
- 文件引用使用 `--link` 蓝色，保留紧凑的文件图标、名称与可选行号；长名称在行内省略。
- 鼠标悬浮或键盘聚焦时显示完整路径，提示浮层允许长路径换行，并限制在视口内。
- 点击文件名直接打开；旁边的省略号菜单提供“打开文件”和“打开所在位置”。图片和视频预览下方复用同一入口。
- 操作进行中禁用重复调用，失败原因显示在该文件下方。保留中英文文案、键盘焦点和明暗主题。
- 普通文档（包括 DOCX、XLSX、PPTX）通过系统关联应用打开；代码与带行号的引用继续使用编辑器。路径与文件类型校验由桌面后端执行。
